...
const SSH_PUB_RE =
  /^\s*(?:ssh-(?:rsa|dss|ed25519)|ecdsa-sha2-nistp(?:256|384|521))\s+[A-Za-z0-9+/]+=*/m;
...
function lineIndexForOffset(lineOffsets: number[], pos: number): number {
  let lo = 0,
...
function scanPattern(
  ...
  switch (pattern.kind) {
    ...
    case "entropy": {
      ...
      const hits = findHighEntropyStrings(
        source,
        threshold,
...
function scanComposite(
  ...
  const perPattern: Array<
    Array<{ start: number; end: number; rawText: string }>
  > = [];
  ...
  for (const anchor of anchors) {
    const allNearby = perPattern
      ...
      .every((others) =>
        others.some(
          (o) =>
            Math.abs(o.start - anchor.start) <= proximity ||
