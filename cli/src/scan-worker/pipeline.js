...
exports.ScanFindingState = void 0;
exports.persistEvaluation = persistEvaluation;
exports.createScanValidationGraph = createScanValidationGraph;
...
// State schema
// ---------------------------------------------------------------------------
exports.ScanFindingState = langgraph_1.Annotation.Root({
    findingId: (0, langgraph_1.Annotation)(),
    repoName: (0, langgraph_1.Annotation)(),
    filePath: (0, langgraph_1.Annotation)(),
    lineNumber: (0, langgraph_1.Annotation)(),
    matchedText: (0, langgraph_1.Annotation)(), // masked
    rawMatchedText: (0, langgraph_1.Annotation)(), // unmasked — never logged
    lineContent: (0, langgraph_1.Annotation)(),
    surroundingContext: (0, langgraph_1.Annotation)(),
    patternId: (0, langgraph_1.Annotation)(),
    templateId: (0, langgraph_1.Annotation)(),
    severity: (0, langgraph_1.Annotation)(),
    // pipeline state
    isHeuristicPlaceholder: (0, langgraph_1.Annotation)(),
    validationStatus: (0, langgraph_1.Annotation)(),
    verdict: (0, langgraph_1.Annotation)(),
    aiReasoning: (0, langgraph_1.Annotation)(),
    confidenceScore: (0, langgraph_1.Annotation)(),
    riskScore: (0, langgraph_1.Annotation)(),
    validationMethod: (0, langgraph_1.Annotation)(),
});
// ---------------------------------------------------------------------------
// KV quota guard
...
async function checkLlmQuota(cache) {
...
async function incrementLlmQuota(cache) {
...
// ---------------------------------------------------------------------------
async function gatherContextNode(state) {
...
// ---------------------------------------------------------------------------
async function heuristicFilterNode(state) {
    const raw = state.rawMatchedText;
    if ((0, scanner_js_1.isLikelyPlaceholder)(raw)) {
        return {
...
function routeAfterHeuristic(state) {
...
// ---------------------------------------------------------------------------
async function apiValidationNode(state) {
    const result = await (0, validator_js_1.validateCredential)(state.patternId, state.rawMatchedText);
...
function routeAfterApiValidation(state) {
...
// ---------------------------------------------------------------------------
async function awsPairReconstructionNode(state) {
    // Only applies to AWS key ID patterns that returned UNVERIFIABLE
    if (state.validationStatus !== "UNVERIFIABLE" ||
        !/^AKIA[0-9A-Z]{16}$/.test(state.rawMatchedText)) {
        return {};
    }
    // Extract secret key from surrounding context — look for common var names
    const secretPattern = /(?:AWS_SECRET_ACCESS_KEY|aws_secret_access_key|secret[_\s]*key|secretKey)\s*[=:'"]+\s*([A-Za-z0-9/+]{40})/;
...
// ---------------------------------------------------------------------------
async function contextInferenceNode(state, env) {
    if (state.validationStatus !== "UNVERIFIABLE")
        return {};
    const pid = state.patternId.toLowerCase();
    const needsContextInference = pid.includes("shopify") ||
...
// ---------------------------------------------------------------------------
async function llmClassificationNode(state, env) {
    const quotaOk = await checkLlmQuota(env.CACHE);
    if (!quotaOk) {
        return {
...
Respond ONLY with valid JSON:
{
  ...
  "confidence": 0.0
    ...
    try {
        ...
        const verdict = parsed.confidence < 0.50
            ? "NEEDS_HUMAN_REVIEW"
...
// Node 5 — Risk Scorer
// ---------------------------------------------------------------------------
async function riskScorerNode(state) {
    const score = (0, types_js_1.findingRiskScore)(state.severity, state.verdict);
    return { riskScore: score };
}
async function persistEvaluation(db, input) {
    const id = crypto.randomUUID();
    await db
        .prepare(`INSERT INTO ai_evaluations
       ...
       ON CONFLICT(finding_id) DO UPDATE SET
        ...
        .bind(id, input.findingId, input.verdict, input.confidence, input.validationMethod, input.validationStatus, input.reasoning)
        .run();
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function impactSummaryNode(state, env) {
    if (state.verdict !== "TRUE_POSITIVE")
        return {};
    const quotaOk = await checkLlmQuota(env.CACHE);
    if (!quotaOk)
        return {};
...
// Compile graph
// ---------------------------------------------------------------------------
function createScanValidationGraph(env) {
    const workflow = new langgraph_1.StateGraph(exports.ScanFindingState)
        ...
        .addConditionalEdges("awsPairReconstruct", (s) => s.verdict === "TRUE_POSITIVE" || s.verdict === "FALSE_POSITIVE"
        ? "riskScorer"
        : "contextInference", { riskScorer: "riskScorer", contextInference: "contextInference" })
        // Context inference: if resolved, skip to scorer; else fall to LLM
        .addConditionalEdges("contextInference", (s) => s.verdict === "TRUE_POSITIVE" || s.verdict === "FALSE_POSITIVE"
        ? "riskScorer"
        : "llmClassification", { riskScorer: "riskScorer", llmClassification: "llmClassification" })
        .addEdge("llmClassification", "riskScorer")
        // After scoring: TRUE_POSITIVE gets impact summary; others terminate
        .addConditionalEdges("riskScorer", (s) => s.verdict === "TRUE_POSITIVE" ? "impactSummary" : "__end__", { impactSummary: "impactSummary", __end__: "__end__" })
