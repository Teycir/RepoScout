...
const SEARCH_QUALIFIERS = 'is:public fork:false';
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function searchPage(token, query, page) {
...
// Main entry point
// ---------------------------------------------------------------------------
async function discoverRepos(env, rawToken) {
    ...
    // ------------------------------------------------------------------
    try {
        await env.DB.prepare(`INSERT INTO crawler_runs (id, started_at, since_cursor, status)
       VALUES (?, datetime('now'), ?, 'RUNNING')`).bind(crawlerRunId, since).run();
    }
    catch (e) {
    ...
    console.log(`[crawler] Run ${crawlerRunId}: +${reposDiscovered} new, ` +
