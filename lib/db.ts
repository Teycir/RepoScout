...
export async function getDashboardStats(db: D1Database): Promise<DashboardStats> {
  const [repos, critical, queue, lastScan] = await Promise.all([
    ...
    db.prepare(
      "SELECT COUNT(*) as n FROM findings f JOIN ai_evaluations e ON e.finding_id = f.id WHERE f.severity IN ('critical','high') AND e.verdict = 'TRUE_POSITIVE'"
    ).first<{ n: number }>(),
    db.prepare(
...
export async function getRepositories(db: D1Database, limit = 50): Promise<RepoRow[]> {
...
export async function getAnalystQueue(db: D1Database, limit = 100): Promise<FindingWithEval[]> {
  const { results } = await db
    .prepare(
      `SELECT f.*, e.id as eval_id, e.verdict, e.confidence, e.validation_method,
              e.validation_status, e.reasoning, e.external_response, e.evaluated_at,
...
export async function getRecentScanRuns(db: D1Database, limit = 10): Promise<ScanRunRow[]> {
  const { results } = await db
    .prepare(
