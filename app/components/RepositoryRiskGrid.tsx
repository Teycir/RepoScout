...
function RiskMeter({ score }: { score: number }) {
  ...
  const config = {
    None:     { bar: 0,    color: 'bg-neon-red/15',  label: 'text-neon-red/30' },
    Low:      { bar: 20,   color: 'bg-neon-red/40',  label: 'text-neon-red/60' },
...
function StatusDot({ status }: { status: string }) {
  if (status === 'COMPLETED') return <CheckCircle size={10} className="text-neon-red/50" />;
  ...
  return (
    <Link
      ...
      className={`group block border rounded-lg p-4 bg-dark-bg/60 backdrop-blur-sm
        ...
        ${level === 'Critical' ? 'glow-red' : level === 'High' ? 'glow-red' : ''}`}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5 text-neon-red/40 text-[10px] mb-0.5">
            <GitBranch size={9} />
            <span className="truncate">{repo.owner}</span>
          </div>
          <h3 className="font-bold text-sm text-white/90 truncate group-hover:text-neon-red transition-colors">
  ...
  if (repos.length === 0) {
    return (
      <div className="border border-neon-red/10 rounded-lg p-12 text-center">
        <div className="text-neon-red/20 text-sm font-mono mb-2">// no repositories monitored yet</div>
        <div className="text-white/20 text-xs">
          Add repos to the D1 <code className="text-neon-red/30">repositories</code> table to start scanning.
        </div>
      </div>
    );
  }
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[10px] font-mono text-neon-red/40 uppercase tracking-widest">
