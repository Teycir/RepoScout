...
function VerdictBadge({
  ...
  if (!verdict) {
    return <span className="text-[9px] text-white/20 font-mono">— pending —</span>;
  }
  if (verdict === 'TRUE_POSITIVE') {
    return (
      <span className="flex items-center gap-1 text-[9px] text-neon-red font-mono">
        ...
        TRUE POSITIVE
        {confidence != null && (
          <span className="text-neon-red/50">({(confidence * 100).toFixed(0)}%)</span>
        )}
      </span>
    );
  }
  ...
  if (verdict === 'NEEDS_HUMAN_REVIEW') {
    return (
      <span className="flex items-center gap-1 text-[9px] text-neon-amber font-mono">
        ...
        NEEDS REVIEW
        {confidence != null && (
          <span className="text-neon-amber/50">({(confidence * 100).toFixed(0)}%)</span>
        )}
      </span>
    );
  }
        ...
        return (
          <div
            ...
            className={`flex gap-2 ${
              isHit ? 'bg-neon-red/8 border-l-2 border-neon-red/50 -mx-3 px-3' : ''
            }`}
          >
...
function FindingCard({ f }: { f: FindingWithEval }) {
  ...
  return (
    <div className={`border rounded-lg p-4 bg-dark-bg/60 backdrop-blur-sm ${borderColor}`}>
      <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
        ...
        <div className="shrink-0 pt-0.5">
          <VerdictBadge verdict={f.eval?.verdict} confidence={f.eval?.confidence} />
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-2 flex-wrap">
        ...
        {f.eval?.validation_method && (
          <span className="text-[9px] text-white/20 font-mono">
            method: <span className="text-white/35">{f.eval.validation_method}</span>
          </span>
        )}
      </div>

      {f.context && <CodeSnippet context={f.context} lineNumber={f.line_number} />}
      ...
      {f.eval?.analyst_reviewed === 1 && f.eval.analyst_verdict && (
        <div className="mt-2 flex items-center gap-1.5 border-t border-white/5 pt-2">
          <span className="text-[9px] text-neon-green/30 font-mono">analyst override:</span>
  ...
  return (
    <div className="border border-neon-green/10 rounded-lg p-5 bg-dark-bg/40 mb-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="text-neon-green/30 text-[10px] font-mono mb-0.5">{owner} /</div>
...
function Section({
  ...
  findings: FindingWithEval[]; dim?: boolean;
}) {
  return (
    <section className={`mb-8 ${dim ? 'opacity-35 hover:opacity-65 transition-opacity duration-300' : ''}`}>
...
export default async function RepoDetailPage({ params }: PageProps) {
  ...
  const truePositives  = sorted.filter((f) => f.eval?.verdict === 'TRUE_POSITIVE');
  const needsReview    = sorted.filter((f) => f.eval?.verdict === 'NEEDS_HUMAN_REVIEW');
  const pending        = sorted.filter((f) => !f.eval);
  const falsePositives = sorted.filter((f) => f.eval?.verdict === 'FALSE_POSITIVE');
    ...
    <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-8">
      ...
      {findings.length === 0 ? (
        <div className="border border-neon-green/10 rounded-lg p-12 text-center">
          <div className="text-neon-green/20 text-sm font-mono mb-2">// no findings for this repository</div>
        ...
        <>
          ...
          {truePositives.length > 0 && <Section title="// confirmed credentials"  count={truePositives.length}  titleColor="text-neon-red/60"   findings={truePositives} />}
          {needsReview.length > 0    && <Section title="// needs analyst review"   count={needsReview.length}    titleColor="text-neon-amber/60" findings={needsReview} />}
          {pending.length > 0        && <Section title="// pending evaluation"     count={pending.length}        titleColor="text-white/20"      findings={pending} />}
          {falsePositives.length > 0 && <Section title="// false positives"        count={falsePositives.length} titleColor="text-white/15"      findings={falsePositives} dim />}
        </>
      )}
    </main>
  );
